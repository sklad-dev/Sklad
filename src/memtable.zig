...
pub const Memtable = struct {
    ...
    level: u8 = 1,
    wal: w.Wal,
    compare_fn: *const fn (data_types.BinaryData, data_types.BinaryData) isize = utils.compareBitwise,
    ...
    pub fn create(allocator: std.mem.Allocator, path: []const u8) !*Memtable {
        ...
        memtable.* = try Memtable.init(
            ...
            config.memtableLevelProbability(),
            path,
        );
        return memtable;
    }

    pub fn fromWal(wal: w.Wal, memtable: *Memtable) !IsFull {
    ...
    pub fn add(self: *Memtable, key: data_types.BinaryData, value: data_types.BinaryData) !void {
        if (!utils.tryLockFor(&self.lock, 200)) return ApplicationError.ExecutionTimeout; // TODO: try to re-queue the task
        defer self.lock.unlock();

        if (self.head == null) try self.createHead();
        ...
        if (self.search(key, path[0..])) |node| {
            ...
            const new_node_level = self.pickLevel();
    ...
    pub inline fn find(self: *Memtable, key: data_types.BinaryData, lock: bool) !?data_types.BinaryData {
        if (lock) {
            if (!utils.tryLockFor(&self.lock, 200)) return ApplicationError.ExecutionTimeout;
        }
        defer {
    ...
    pub inline fn isFull(self: *const Memtable) bool {
...
fn visualizeMemtable(table: Memtable) !void {
...
test "Memtable#add and find" {
    ...
    try testing.expect(try test_memtable.find(&utils.intToBytes(u8, 1), false) == null);

    // Case: a key is added succesfully to an empty memtable
    const test_value = utils.intToBytes(u8, 0);
    const test_key0 = utils.intToBytes(u8, 0);
    ...
    const test_key1 = utils.intToBytes(u8, 0);
    ...
    var table_size: u8 = 1;
    for (0..16) |_| {
        try test_memtable.add(&utils.intToBytes(u8, table_size), &test_value);
        table_size += 1;
        try testing.expect(test_memtable.size == table_size);
    }

    // Case: find
    try testing.expect(try test_memtable.find(&utils.intToBytes(u8, 0), false) != null);
    try testing.expect(try test_memtable.find(&utils.intToBytes(u8, table_size / 2), false) != null);
    try testing.expect(try test_memtable.find(&utils.intToBytes(u8, table_size + 1), false) == null);
