...
fn compare_bitwise(v1: []const u8, v2: []const u8) isize {
    if (v1.len == v2.len and std.mem.eql(u8, v1, v2)) {
        return 0;
    }
...
fn size(comptime N: usize, table: SkipListMemtable(N)) i32 {
    var result: i32 = -1;
...
fn visualize_memtable(comptime N: usize, table: SkipListMemtable(N)) !void {
...
test "SkipListMemtable#add and find" {
