...
pub const Arena = struct {
    ...
    pub inline fn currentOffset(self: *Arena) u64 {
...
pub const Memtable = struct {
    ...
    pub inline fn init(allocator: std.mem.Allocator, random: std.Random, max_size: u64, max_level: u8, wal_path: []const u8) !Memtable {
        ...
        _ = arena.reserve(node_and_tower_size);
        ...
        const tail_offset_start = arena.reserve(node_and_tower_size);
    ...
    pub fn add(self: *Memtable, key: data_types.BinaryData, value: data_types.BinaryData) !void {
        ...
        while (true) {
            ...
            if (found_index != 0) {
                ...
                const value_offset = self.arena.reserve(value.len);
                ...
                if (!created) {
                    new_node_offset = self.arena.reserve(@sizeOf(Node) + (new_node_height + 1) * @sizeOf(u64));
                    ...
                    const key_value_offset = self.arena.reserve(key.len + value.len);
    ...
    pub fn find(self: *Memtable, key: data_types.BinaryData) ?data_types.BinaryData {
    ...
    pub inline fn isFull(self: *const Memtable) bool {
        return @atomicLoad(u16, &self.arena.currentOffset(), .seq_cst) >= self.max_size;
    }
    ...
    fn search(self: *Memtable, key: data_types.BinaryData, predecessors: ?[]u64, successors: ?[]u64) u64 {
        var pred: ?*Node = null;
...
fn visualizeMemtable(memtable: *Memtable) void {
    ...
    while (node_offset < memtable.max_size) {
        ...
        std.debug.print("{any}:\t", .{curr.value});
        for (curr.tower) |j| {
            std.debug.print("{d}\t", .{j});
        }
        std.debug.print("\n", .{});
...
test "Arena" {
    var arena = try Arena.init(testing.allocator, 32);
    defer arena.deinit();

    var o: u64 = arena.reserve(@sizeOf(u64));
    try testing.expect(o == 0);
    o = arena.reserve(@sizeOf(u128));
    try testing.expect(o == 8);
    o = arena.reserve(@sizeOf(u64));
...
//             const offset = arena.reserve(128);
