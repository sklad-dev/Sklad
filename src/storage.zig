...
pub const Storage = struct {
    ...
    active_memtable: ?*mt.Memtable,
    memtables: ArrayList(*mt.Memtable),
    table_files: AutoHashMap(u8, *ArrayList([]u8)),
    tables: StringHashMap(*st.SSTable),

    pub fn start(path: []const u8, max_memtable_size: u16, allocator: std.mem.Allocator) !Storage {
        ...
        var storage = Storage{
            ...
            .memtables = try ArrayList(*mt.Memtable).initCapacity(allocator, 2),
    ...
    pub fn write(self: *Storage, comptime T: type, value: T) !void {
    ...
    pub fn find(self: *Storage, key: []const u8) !?u64 {
    ...
    fn find_in_tables(self: *Storage, key: []const u8) !?u64 {
    ...
    fn add_memtable(self: *Storage) !void {
        var rng = std.rand.DefaultPrng.init(blk: {
            var seed: u64 = undefined;
            try std.posix.getrandom(std.mem.asBytes(&seed));
            break :blk seed;
        });
        const memtable = try self.allocator.create(mt.Memtable);
        memtable.* = try mt.Memtable.init(
            self.allocator,
            rng.random(),
            8,
    ...
    fn restore_memtables(self: *Storage) !void {
    ...
    fn restore_memtable(self: *Storage, wal: w.Wal) !void {
        var rng = std.rand.DefaultPrng.init(blk: {
            var seed: u64 = undefined;
            try std.posix.getrandom(std.mem.asBytes(&seed));
            break :blk seed;
        ...
        const memtable = try self.allocator.create(mt.Memtable);
        memtable.* = try mt.Memtable.from_wal(
            wal,
            self.allocator,
            rng.random(),
            8,
    ...
    fn add_table_file_at_level(self: *Storage, level: u8, table_file: []u8) !void {
    ...
    fn map_sstable_files(self: *Storage) !void {
        var dir = try std.fs.cwd().openDir(self.path, .{
            .access_sub_paths = false,
            .iterate = true,
            .no_follow = true,
    ...
    fn deinit_memtables(self: *Storage) void {
    ...
    fn deinit_table_files(self: *Storage) void {
    ...
    fn deinit_table_levels(self: *Storage) void {
...
fn clean_up(storage: *Storage) !void {
...
test "Add value" {
    ...
    var test_storage = try Storage.start("./", 4, testing.allocator);
...
test "Restore memtable from wal" {
    ...
    var storage1 = try Storage.start("./", 4, testing.allocator);
    try storage1.write(u8, 1);
    storage1.stop();

    var storage2 = try Storage.start("./", 4, testing.allocator);
    defer storage2.stop();
...
test "Finding values" {
    var storage = try Storage.start("./", 4, testing.allocator);
    defer storage.stop();
